...
import { useReceptionStats } from '@/hooks/useReceptionStats';
...
export function ReceptionTab({ rallyActionsMap, players, match, selectedSet }: ReceptionTabProps) {
  const [sideFilter, setSideFilter] = useState<Side>('CASA');

  const stats = useReceptionStats(rallyActionsMap, players, { side: sideFilter, selectedSet });
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="p-0 overflow-x-auto">
          {stats.length === 0 ? (
            <div className="text-center text-muted-foreground py-8 text-sm">Sem dados de receção</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-center">✗ Q0</TableHead>
              ...
              <TableBody>
                {stats.map(p => (
                  <TableRow key={p.playerId}>
                    ...
                    <TableCell className="text-center"><QualityCell count={p.q3} total={p.total} /></TableCell>
                    <TableCell className="text-center"><QualityCell count={p.q2} total={p.total} /></TableCell>
                    <TableCell className="text-center"><QualityCell count={p.q1} total={p.total} /></TableCell>
                    <TableCell className="text-center"><QualityCell count={p.q0} total={p.total} /></TableCell>
